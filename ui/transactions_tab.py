...
class TransactionsTab:
    ...
    def refresh(self) -> None:
        ...
        for transaction in self.app.tracker.transactions:
    ...
    def _add_transaction(self) -> None:
        try:
            ...
            if self.editing_transaction_id:
                self.app.tracker.update_transaction(
                ...
                self.app.tracker.add_income(amount, category, description, transaction_date)
                message = "Income added and saved."
            else:
                self.app.tracker.add_expense(amount, category, description, transaction_date)
    ...
    def _delete_selected_transaction(self) -> None:
        try:
            ...
            self.app.tracker.delete_transaction_by_id(transaction_id)
            self.app.tracker.save()
            if self.editing_transaction_id == transaction_id:
                self._reset_transaction_form()
            self.app.refresh()
            self.app.set_status("Transaction deleted and saved.")
    ...
    def _edit_selected_transaction(self) -> None:
        try:
            transaction_id = self._selected_transaction_id()
            transaction = self.app.tracker.get_transaction_by_id(transaction_id)
        except ValueError as error:
        ...
        self.app.set_status("Editing selected transaction.")
